...
void validate_shapes(const Tensor& hidden,
    ...
    const bool has_qk_norm_gate = w.q_norm_weight != nullptr && w.k_norm_weight != nullptr;
...
void run_full_attention_core(const Tensor& hidden,
    ...
    const bool has_qk_norm_gate = weights.q_norm_weight != nullptr && weights.k_norm_weight != nullptr;
    ...
    const int64_t QProjOut = has_qk_norm_gate ? QMainDim * 2 : QMainDim;
    ...
    if (weights.input_norm_weight->shape() != std::vector<int64_t>{Hidden} ||
        ...
        (has_qk_norm_gate && (weights.q_norm_weight->shape() != std::vector<int64_t>{HeadDim} ||
    ...
    Tensor q_attn({T, QMainDim}, DType::Float16); q_attn.allocate();
    Tensor q_gate;
    if (has_qk_norm_gate) {
        q_gate = Tensor({T, QMainDim}, DType::Float16); q_gate.allocate();
        split_q_gate(q_full, NumQHeads, HeadDim, q_attn, q_gate, stream);
    } else {
        copy_matrix_rows(q_full, 0, q_attn, 0, T, stream);
    }
    ...
    copy_heads_from_cols(q_attn, NumQHeads, HeadDim, q_heads, stream);
...
DecodeState make_decode_state(int64_t max_seq_len,
    ...
    for (const auto& type : layer_types) {
        if (type == "full_attention" || type == "llama_attention") {
            FullAttentionLayerCache cache;
...
void full_attention_decoder_layer_step(const Tensor& hidden,
    ...
    Tensor q_attn({1, QMainDim}, DType::Float16); q_attn.allocate();
    Tensor q_gate;
    if (has_qk_norm_gate) {
        q_gate = Tensor({1, QMainDim}, DType::Float16); q_gate.allocate();
        split_q_gate(q_full, NumQHeads, HeadDim, q_attn, q_gate, stream);
    } else {
        copy_matrix_rows(q_full, 0, q_attn, 0, 1, stream);
    }
