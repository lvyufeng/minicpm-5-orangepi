...
- **Web Demo**（`app.py` + `index.html`）：Python 侧使用 Hugging Face tokenizer 组织 chat template 和解码文本，底层从常驻 C++ 后端流式获取 token。
...
```text
src/csrc/include/minicpmv/   C++ runtime 头文件
src/csrc/lib/                C++ runtime 实现
src/csrc/custom_ops/         Ascend 自定义算子 host/kernel 代码
src/csrc/tools/              CLI 工具：minicpm5_decode、minicpm5_server
scripts/                     环境、构建和 demo 脚本
app.py                       Web Demo 的 FastAPI 封装
index.html                   浏览器聊天 UI
requirements.txt             Web Demo 的 Python 依赖
```
...
- 带 Ascend CANN 环境的香橙派 / Linux 设备。
- Ascend toolkit 位于默认路径，或通过 `MINICPM5_ASCEND_TOOLKIT_ROOT` / `MINICPMV_ASCEND_TOOLKIT_ROOT` 指定。
- 本地已准备 MiniCPM5-1B 权重，通常放在：

```text
models/MiniCPM5-1B
```

- Web Demo 需要 Python 3.9+。
...
`minicpm5_server` 是一个长运行 C++ 进程。它加载一次权重后，从 stdin 接收换行分隔请求：

```text
REQUEST <max_new> <comma_separated_input_ids>
```

stdout 会逐行流式输出生成的 token ID，并以：
